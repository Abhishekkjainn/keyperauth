...
export default function Authpage() {
  const [activeTab, setActiveTab] = useState('signin'); // Default active tab
  const [phone, setPhone] = useState(Array(10).fill('')); // Array to store 10 digits
  const inputRefs = useRef([]); // Ref for handling focus
  ...
  const handlePaste = (event) => {
    ...
    if (pasteData.length === 10) {
      const newPhone = pasteData.split('');
      setPhone(newPhone);
    ...
    if (hiddenInput) {
      const handleAutofill = (event) => {
        ...
        if (autofillValue.length === 10) {
          const newPhone = autofillValue.split('');
          setPhone(newPhone);
            ...
            <div className="authform">
              ...
              <div className="buttonsection">
                ...
                <div className="button1">
                  {' '}
                  <img src="/signin.png" alt="" className="signinicon" />{' '}
                  Register
                </div>
              </div>
            </div>
